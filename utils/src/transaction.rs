...
impl TransactionWrapper<'_> {
    ...
    // check tx and if works process it
    pub async fn try_and_process_tx(&self, ptx: ProgrammableTransaction,
                       gascoin_object_ref: Option<ObjectRef>,
                       gasprice: Option<u64>,
                       gasbudget: Option<u64>,) -> Result<TransactionDigest, Error> {
        ...
        let result = self.client.read_api().dry_run_transaction_block(tx_data.clone()).await;
        if result.is_err() {
